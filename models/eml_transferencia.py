...
class EMLTransferencia(models.Model):
    ...
    num_proveedor = fields.Char(related='partner_id.ref', string='Número de Proveedor')
    account_type = fields.Char(string='Tipo de Cuenta')
    account_id = fields.Many2one(related='partner_id.property_account_receivable_id', string='Número de Cuenta Contable')
    partner_id = fields.Many2one('res.partner', string='Beneficiario')
    partner_id_vat = fields.Char(related='partner_id.vat', string='RFC')
    partner_id_accounts = fields.Many2one('res.partner.bank', string='Cuentas de Banco')
    partner_id_acc_bank = fields.Char(related='partner_id_accounts.bank_name', string='Banco', readonly=True)
    partner_id_acc_c_bank = fields.Char(related='partner_id_accounts.bank_id.l10n_mx_edi_code', string='Código', readonly=True)
    partner_id_acc_number = fields.Char(related='partner_id_accounts.acc_number', string='Número de cuenta', readonly=True)
    partner_id_acc_clabe = fields.Char(related='partner_id_accounts.l10n_mx_edi_clabe', string='CLABE', readonly=True)
    partner_id_acc_currency = fields.Many2one(related='partner_id_accounts.currency_id', string='Moneda', readonly=True)
    ...
    country_id = fields.Many2one(related='partner_id.country_id', string='País')
